...
; Modify-date: 08.08.2008
;********************************************************************
; Modified by LarsJ
; Modify-date: 19.10.2014
; Modify-date: 24.01.2015, updates suggested by nickston
;********************************************************************
...
#cs
	...
	*** GUI menu item commands ***
	...
	_GUICtrlCreateODTopMenu($sText, $nParentGUI) ; <--- ProgAndy
	...
	_GUIMenuBarSetBkColor($nParentGUI,$nMenuBkClr) ; <--- ProgAndy
...
; ------------------------------
; _AddMenuIcon($sIconFile, $nIconID)
; _ColorGetClr($nColor, $nMode)
; _CreateBlankIcon()
; _CreateMenuFont(ByRef $hFont, $bBold = FALSE, $bSide = FALSE)
; _CreateMenuFontByName($sFontName, $nHeight = 8, $nWidth = 400)
; _CreateSideMenu($nMenuID)
; _DeleteSideMenu($nIdx)
; _FillGradientRect($hDC, $stRect, $nClr1, $nClr2, $bVert = FALSE)
; _GetBGRColor($nColor)
; _GetIconID($nID, $sFile)
; _GetMenuHandle($nMenuItemID)
; _GetMenuIconIndex($nMenuItemID, ByRef $nIconIndex, ByRef $nSelIconIndex)
; _GetMenuIndex($hMenu, $nMenuItemID)
; _GetMenuInfos(ByRef $nS, ByRef $nX)
; _GetMenuIsRadio($nMenuItemID)
; _GetMenuMaxTextWidth($hDC, $hMenu, ByRef $nMaxWidth, ByRef $nMaxAccWidth)
; _GetMenuText($nMenuItemID)
; _GetNewTrayIndex()
; _GetNewItemIndex()
; _GetNewSideItemIndex()
; _GetSideMenuColors($nIdx, ByRef $nColor, ByRef $nBkColor, ByRef $nBkGradColor)
; _GetSideMenuImage($nIdx)
; _GetSideMenuImageSize($nIdx, ByRef $nWidth, ByRef $nHeight, ByRef $bStretch)
; _GetSideMenuIndex($hMenu)
; _GetSideMenuText($nIdx)
; _GetSideMenuTextWidth($hDC, $sText)
; _GetTrayNotifyIdx($nID)
...
; _GUICtrlODMenuItemSetSelIcon($nMenuID, $sIconFile = "", $nIconID = 0)
; _GUICtrlODMenuItemSetText($nMenuID, $sText)
; _GUIMenuBarSetBkColor($nParentGUI,$nMenuBkClr) ; <--- ProgAndy
; _IsSideMenuIdx($nIdx)
; _IsTrayItem($nMenuItemID)
; _ReplaceMenuIcon($sIconFile, $nIconID, $nReplaceIndex)
; _SetFlashTimeOut($nTime = 750)
; _SetItemRect(ByRef $stStruct, $p1, $p2, $p3, $p4)
; _SetMenuBkColor($nColor)
; _SetMenuIconBkColor($nColor)
; _SetMenuIconBkGrdColor($nColor)
; _SetMenuSelectBkColor($nColor)
; _SetMenuSelectRectColor($nColor)
; _SetMenuSelectTextColor($nColor)
; _SetMenuTextColor($nColor)
; _SetOwnerDrawn($hMenu, $MenuItemID, $sText, $bOwnerDrawn = TRUE)
; _SetSideMenuBkColor($nIdx, $nColor = -2)
; _SetSideMenuBkGradColor($nIdx, $nColor = -2)
; _SetSideMenuColor($nIdx, $nColor = -2)
; _SetSideMenuColors($nIdx, $nColor = -1, $nBkColor = -1, $nBkGrdColor = -1)
; _SetSideMenuImage($nIdx, $sFile, $sResName = "", $bStretch = FALSE)
; _SetSideMenuText($nIdx, $sText, $sResName = "", $bIsBitmap = FALSE, $bStretch = FALSE)
; _SetTrayBkColor($nColor)
; _SetTrayIconBkColor($nColor)
; _SetTrayIconBkGrdColor($nColor)
; _SetTraySelectBkColor($nColor)
; _SetTraySelectRectColor($nColor)
; _SetTraySelectTextColor($nColor)
; _SetTrayTextColor($nColor)
...
; _TrayDeleteItem($nID)
; _TrayGetMenuHandle($nID)
; _TrayIconCreate($sToolTip = "", $sIconFile = @AutoItExe, $nIconID = 0, $sCallback = "", $nMsg = 0, $hIcon = 0)
; _TrayIconDelete($nID)
; _TrayIconSetClick($nID, $nClicks)
; _TrayIconSetIcon($nID = -1, $sIconFile = @AutoItExe, $nIconID = 0)
; _TrayIconSetState($nID = -1, $nState = 1)
; _TrayIconSetToolTip($nID = -1, $sToolTip = $sDefaultTT)
; _TrayItemSetIcon($nMenuID, $sIconFile = "", $nIconID = -1)
; _TrayItemSetSelIcon($nMenuID, $sIconFile = "", $nIconID = -1)
; _TrayItemSetText($nMenuID = -1, $sText = "")
; _TrayNotifyIcon($hWnd, $Msg, $wParam, $lParam)
; _TrayTip($nID, $sTitle, $sText, $nTimeOut = 10, $nInfoFlags = 0)
; _TrayWndProc($hWnd, $Msg, $wParam, $lParam)
...
Func _GetNewTrayIndex()
	...
	If Not $bFreeFound Then
		...
		;If $TRAYNOTIFYIDS[0][0] > $nSize - 10 Then _ ; nickston
	...
	If BitAnd($nState, 4) Then
		If Not $TRAYNOTIFYIDS[$nID][7] Then
			...
			If $hBlankIcon <> 0 Then
				;SetTimer($TRAYMSGWND, $FLASHTIMERID, $FLASHTIMEOUT, 0) ; nickston
...
Func _GUIMenuBarSetBkColor($nParentGUI,$nMenuBkClr)
	...
	DllCall("User32.dll", "int", "RedrawWindow", "hwnd", $nParentGUI, "ptr", 0, "handle", 0, "uint", 0x0400)
...
Func _CreateMenuFont(ByRef $hFont, $bBold = FALSE, $bSide = FALSE)
	...
	If SystemParametersInfo(0x0029, DllStructGetSize($stNCM), DllStructGetPtr($stNCM), 0) Then
		...
		Local $SegUIVarExists2 = FileExists($sSegUIVar2)

		If $SegUIVarExists2 Then
			If $hFont <> 0 Then $hFont = _CreateMenuFontByName("Segoe UI Variable Display")
		Else
			If $hFont <> 0 Then $hFont = _CreateMenuFontByName("Segoe UI")
		EndIf
	EndIf
EndFunc
...
Func MulDiv($nInt1, $nInt2, $nInt3)
	Local $nResult = DllCall("kernel32.dll", "int", "MulDiv", _
											"int", $nInt1, _
											"int", $nInt2, _
											"int", $nInt3)
	Return $nResult[0]
...
Func LoadLibraryExW($pFile, $hFile, $nFlags)
	Local $hResult = DllCall($hKernel32Dll, "hwnd", "LoadLibraryExW", _
												"ptr", $pFile, _
												"hwnd", $hFile, _
												"dword", $nFlags)
	Return $hResult[0]
...
Func FreeLibrary($hLib)
	Local $nResult = DllCall($hKernel32Dll, "int", "FreeLibrary", _
												"hwnd", $hLib)
	Return $nResult[0]
EndFunc
