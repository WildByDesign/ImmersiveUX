...
Func ObjectFromTag($sFunctionPrefix, $tagInterface, ByRef $tInterface, $fPrint = False, $bIsUnknown = Default, $sIID = "{00000000-0000-0000-C000-000000000046}") ; last param is IID_IUnknown by default
	...
	ConsoleWrite("tag: " & $tagInterface & @CRLF)
	...
	For $i = 0 To $iUbound - 1
		...
		ConsoleWrite("$sMethod: " & $sMethod & @CRLF)
		ConsoleWrite("$sRet: " & $sRet & @CRLF)
		ConsoleWrite("$sParams: " & $sParams & @CRLF)
	...
	$hCallback = DllCallbackRegister("UIAEH_AutomationEventHandler_QueryInterface", "long", "ptr;ptr;ptr")
	DllStructSetData($tInterface, "Methods", DllCallbackGetPtr($hCallback), 1)
	DllStructSetData($tInterface, "Callbacks", $hCallback, 1)

	$hCallback = DllCallbackRegister("UIAEH_AutomationEventHandler_AddRef", "dword", "ptr")
	DllStructSetData($tInterface, "Methods", DllCallbackGetPtr($hCallback), 2)
	DllStructSetData($tInterface, "Callbacks", $hCallback, 2)

	$hCallback = DllCallbackRegister("UIAEH_AutomationEventHandler_Release", "dword", "ptr")
	DllStructSetData($tInterface, "Methods", DllCallbackGetPtr($hCallback), 3)
	DllStructSetData($tInterface, "Callbacks", $hCallback, 3)

	$hCallback = DllCallbackRegister("UIAEH_AutomationEventHandler_HandleAutomationEvent", "long", "ptr;ptr;int")
	DllStructSetData($tInterface, "Methods", DllCallbackGetPtr($hCallback), 4)
	DllStructSetData($tInterface, "Callbacks", $hCallback, 4)
