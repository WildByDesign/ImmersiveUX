...
;
; Revisions
;  09/19/2022 ...: - Fixed issue, added update from 05/25/2022 that was accidentally removed
;  09/01/2022 ...: - Added $BS_MULTILINE button style
;				   - Added ellipses when text is longer than the button
;				   - Fixed compatibility with Opt("MustDeclareVars", 1)
;  05/25/2022 ...: - Fixed issue, buttons disappear when a GUI containing a window with WS_EX_MDICHILD is moved
...
;				   $style		  - Button style ($BS_TOP, $BS_BOTTOM, $BS_LEFT, $BS_RIGHT, $BS_TOOLBUTTON, $BS_MULTILINE)
...
Func GuiFlatButton_Create($sText, $x, $y, $w = -1, $h = -1, $style = 0)
	...
	Local $childHWND = GUICreate("", $w, $h, $x, $y, BitOR($WS_CHILD, $WS_CLIPSIBLINGS), $WS_EX_CONTROLPARENT, $parentHWND)
	_WinAPI_SetWindowPos($childHWND, $HWND_TOP, 0, 0, 0, 0, BitOR($SWP_NOMOVE, $SWP_NOSIZE))
	GUISetState(@SW_SHOWNOACTIVATE)
	;create the new button
	Local $buttonID = GUICtrlCreateButton("", 0, 0, $w, $h)
	...
	Local $idIndex = GuiFlatButton_FindControlId($buttonID)
	If $idIndex <> -1 Then
	...
	$aGuiFlatButton[$i][21] = $x
	$aGuiFlatButton[$i][22] = $y
	$aGuiFlatButton[$i][23] = False
...
Func GuiFlatButton_SetPos($controlID, $iLeft, $iTop = Default, $iWidth = Default, $iHeight = Default)
	...
	$controlHWND = GUICtrlGetHandle($aGuiFlatButton[$ControlIndex][1])
	$childHWND = _WinAPI_GetParent($controlHWND)
	$aGuiFlatButton[$ControlIndex][23] = True ;set our flag to allow moving
	WinMove($childHWND, "", $iLeft, $iTop, $iWidth, $iHeight)
	GUICtrlSetPos($aGuiFlatButton[$ControlIndex][1], 0, 0, $iWidth, $iHeight)
	$aGuiFlatButton[$ControlIndex][21] = $iLeft
	$aGuiFlatButton[$ControlIndex][22] = $iTop
	$aGuiFlatButton[$ControlIndex][23] = False ;reset our flag to disable moving
...
Func GuiFlatButton_ChildHandler($hwnd, $iMsg, $wParam, $lParam, $idx, $pData)
	If $iMsg <> $WM_DRAWITEM And $iMsg <> $WM_NCHITTEST And $iMsg <> $WM_DESTROY And $iMsg <> $WM_WINDOWPOSCHANGING Then Return DllCall("comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hwnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam)[0]
	...
	Switch $iMsg
		...
		Case $WM_DESTROY
			...
			;this is for handling a strange phenomenon when WS_EX_MDICHILD is used, the child button is moved way off screen
			;'reset' the X and Y position unless we are setting the position ourselves.
		Case $WM_WINDOWPOSCHANGING
			If Not $aGuiFlatButton[$idx][23] Then
				Local $tWindowPos = DllStructCreate($tagWINDOWPOS, $lParam)
				DllStructSetData($tWindowPos, "X", $aGuiFlatButton[$idx][21])
				DllStructSetData($tWindowPos, "Y", $aGuiFlatButton[$idx][22])
...
Func GuiFlatButton_DrawButton($hwnd, $hCtrl, $hDC, $iLeft, $iTop, $iRight, $iBottom, $nItemState, $idx)
	...
	Local $bMultiLine = BitAND($aGuiFlatButton[$idx][20], $BS_MULTILINE) = $BS_MULTILINE
	...
	;set the text flags and calculate text size
	Local $iFlags = BitOR($DT_NOCLIP, $DT_WORD_ELLIPSIS)
	If Not $bLeft And Not $bRight Then $iFlags = BitOR($iFlags, $DT_CENTER)
	If $bLeft Then $iFlags = BitOR($iFlags, $DT_LEFT)
	If $bRight Then $iFlags = BitOR($iFlags, $DT_RIGHT)
	If Not $bMultiLine Then $iFlags = BitOR($iFlags, $DT_SINGLELINE)
	...
	_WinAPI_DrawText($hDC, $sText, $tTextRect, BitOR($iFlags, $DT_CALCRECT))

	;text position setup
	Local $iTextRectLeft = DllStructGetData($tTextRect, 'Left')
	Local $iTextRectTop = DllStructGetData($tTextRect, 'Top')
	Local $iTextRectRight = DllStructGetData($tTextRect, 'Right')
	Local $iTextRectBottom = DllStructGetData($tTextRect, 'Bottom')
...
;~ 					_WinAPI_DrawIconEx($hDC, $iIconX, $iIconY, $aGuiFlatButton[$idx][19], $bmWidth, $bmHeight)
	...
	If $bBottom Then
		If $bLeft Then
			$iTextRectLeft = $bmWidth + 2 + 1
			$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
			If $useTextHeight Then
				$iTextRectTop = $iBottom - $iTextHeight - 2
			Else
				$iTextRectTop = $iBottom - 2 - $bmHeight / 2 - $iTextHeight / 2
			EndIf
			$iTextRectBottom = $iTextRectTop + $iTextHeight
		ElseIf $bRight Then
			$iIconX = $iRight - $bmWidth - 2
			$iTextRectLeft = $iRight - $iTextWidth - $bmWidth - 2 - 1
			$iTextRectRight = $iTextRectLeft + $iTextWidth
			If $useTextHeight Then
				$iTextRectTop = $iBottom - $iTextHeight - 2
			Else
				$iTextRectTop = $iBottom - 2 - $bmHeight / 2 - $iTextHeight / 2
			...
			$iTextRectBottom = $iTextRectTop + $iTextHeight
		Else
			$iTextRectLeft = ($iRight - $iLeft) / 2 - ($iTextWidth) / 2
			$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
			$iTextRectTop = $iBottom - $bmHeight - 2 - $iTextHeight - 1
			$iTextRectBottom = $iTextRectTop + $iTextHeight
		EndIf
	ElseIf $bTop Then
		If $bLeft Then
			...
			If $useTextHeight Then
				$iTextRectTop = $iTop + 2
			Else
				$iTextRectTop = $iTop + 2 + $bmHeight / 2 - $iTextHeight / 2
			EndIf
			$iTextRectBottom = $iTextRectTop + $iTextHeight
		ElseIf $bRight Then
			$iIconX = $iRight - $bmWidth - 2
			$iTextRectLeft = $iRight - $iTextWidth - $bmWidth - 2 - 1 - 1
			$iTextRectRight = $iTextRectLeft + $iTextWidth
			If $useTextHeight Then
				$iTextRectTop = $iTop + 2
			Else
		...
		Else
			$iTextRectLeft = ($iRight - $iLeft) / 2 - ($iTextWidth) / 2
			$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
			$iTextRectTop = $iTop + $bmHeight + 2 + 1
			$iTextRectBottom = $iTextRectTop + $iTextHeight
		EndIf
	ElseIf $bLeft Then
		$iTextRectLeft = $bmWidth + 2 + 1
		$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
		$iTextRectTop = $iTop + ($iBottom - $iTop) / 2 - $iTextHeight / 2
		$iTextRectBottom = $iTextRectTop + $iTextHeight
	ElseIf $bRight Then
		$iIconX = $iRight - $bmWidth - 2
		$iTextRectLeft = $iRight - $iTextWidth - $bmWidth - 2 - 1
		...
		$iTextRectBottom = $iTextRectTop + $iTextHeight
	ElseIf $bToolButton Then
		$iTextRectLeft = $iLeft + ($iRight - $iLeft) / 2 - $iTextWidth / 2
		$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
		$iTextRectTop = ($iBottom - $iTop) / 2 - ($iTextHeight + $bmHeight) / 2 + $bmHeight + 1
		$iTextRectBottom = $iTextRectTop + $iTextHeight
	Else
		$iTextRectLeft = $iLeft + ($iRight - $iLeft) / 2 - ($iTextWidth + $bmWidth + 2) / 2 + $bmWidth + 1
		$iTextRectRight = $iRight - ($iTextRectLeft - $iLeft)
		$iTextRectTop = $iTop + ($iBottom - $iTop) / 2 - $iTextHeight / 2
		$iTextRectBottom = $iTextRectTop + $iTextHeight
	EndIf
	...
	If $bSelected Then
		$iTextRectLeft = $iTextRectLeft + 1
		$iTextRectRight = $iTextRectRight + 1
;~ 		$iTextRectTop = $iTextRectTop + 2
	EndIf

	DllStructSetData($tTextRect, 'Left', $iTextRectLeft)
	DllStructSetData($tTextRect, 'Top', $iTextRectTop)
	DllStructSetData($tTextRect, 'Right', $iTextRectRight)
	DllStructSetData($tTextRect, 'Bottom', $iTextRectBottom)
