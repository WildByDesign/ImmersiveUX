...
;                                                      4 = Enable Debugging. The COM errors will be handled without any output
...
;                  This function calls _TS_ErrorNotify(4) to make sure COM errors are handled. For details please see _TS_ErrorNotify
; Related .......:
; Link ..........:
; Example .......: Yes
; ===============================================================================================================================
...
; Parameters ....: $oService - Object of the Task Scheduler Service created by _TS_Open
...
Func _TS_ErrorText($iErrorNumber, $bPrefix = True)
	...
	Local $aErrorMessages[][] = [ _
			...
			["_TS_TaskExportXML", 1502, "Error opening the output file. @extended is set to @error as returned by FileOpen"], _
			["_TS_TaskExportXML", 1503, "Error retrieving the XML code. @extended is set to the COM @error code"], _
			...
			["_TS_TaskImportXML", 1705, "The file specified by $vXMLInput does not exist or there was an error reading the file"], _
			...
			["_TS_TaskPropertiesSet", 2005, "Error setting a property. @extended is set to the COM error and the Returnvalue holds the negative index of the property in error ($aProperties)"], _
...
; #FUNCTION# ====================================================================================================================
; Name ..........: _TS_IsRunFromTS
; Description ...: Check if process is run by Task Scheduler.
; Syntax ........: _TS_IsRunFromTS($oService[, $iPID = @AutoItPID[, $bCheckPath = False]])
; Parameters ....: $oService   - Task Scheduler Service object as returned by _TS_Open
...
;                  |3402 - __TS_ProcessParentPID returned an error. @extended has been set to this error code.
; Author ........: mLipok
; Modified ......: water
; Remarks .......:
; Related .......:
...
; Example .......: Yes
; ===============================================================================================================================
Func _TS_IsRunFromTS($oService, $iPID = @AutoItPID, $bCheckPath = False)
    ...
    Local $iParentPID = __TS_ProcessParentPID($iPID)
    ...
    For $i = 0 To UBound($aRunningTasks) - 1
        ...
        If $aRunningTasks[$i][1] = $iPID And $bTestingPath Then Return True
        If $aRunningTasks[$i][1] = $iParentPID And $bTestingPath Then Return True
    Next
    Return False
...
; Author ........: water
; Modified.......:
; Remarks .......: This function exports the TaskDefinition of the specified Task to a local array using _TS_TaskExportXML.
;                  This array is then used to create the new Task Definition which is then returned to the caller.
;                  The XML export by _TS_TaskExporXML does not contain the password if the LogonType is set to $TASK_LOGON_PASSWORD
;                  or any other logon type that requires a password. The password must be supplied at registration time.
;                  You should modify the Task Definition if needed before calling _TS_TaskRegister to create a new Task.
;                  Calling _TS_TaskRegister is mandatory to create the new Task!
; Related .......:
; Link ..........:
; Example .......: Yes
; ===============================================================================================================================
...
;                  |1502 - Error opening the output file. @extended is set to @error as returned by FileOpen
;                  |1503 - Error retrieving the XML code. @extended is set to the COM @error code
; Author ........: water
; Modified.......:
...
Func _TS_TaskExportXML($oService, $sTask, $sXMLOutput = Default)
	...
	Local $sXML = $oTask.XML
	If @error Then Return SetError(1503, @error, 0)
	If $sXMLOutput = "" Then
...
; Parameters ....: $oService    - Task Scheduler Service object as returned by _TS_Open
;                  $sTaskPath   - Task path (Folder plus Task name) to process
;                  $bReturnFlag - [optional] Set to 1 if the function should return the TaskDefinition object (default = Task object)
...
; Syntax.........: _TS_TaskImportXML($oService, $iInputType, $vXMLInput)
; Parameters ....: $oService   - Task Scheduler Service object as returned by _TS_Open
...
;                  |1705 - The file specified by $vXMLInput does not exist or there was an error reading the file
; Author ........: water
; Modified.......:
; Remarks .......: The XML export by _TS_TaskExporXML does not contain the password if the LogonType is set
;                  to $TASK_LOGON_PASSWORD or any other logon type that requires a password. The password must be supplied at registration time.
;                  You should modify the Task Definition if needed before calling _TS_TaskRegister to create a new Task.
; Related .......:
; Link ..........:
; Example .......: Yes
...
Func _TS_TaskImportXML($oService, $iInputType, $vXMLInput)
	...
	If $iInputType = 1 Then
		If FileExists($vXMLInput) <> 1 Then Return SetError(1705, @error, 0)
		Local $sXML = FileRead($vXMLInput)
		If @error Then Return SetError(1705, @error, 0)
	...
	If @error = 0 Then
		...
		For $i = 1 To $iTaskCount
			...
			If BitAND($iProperties, 2 ^ 2) = 2 ^ 2 Then
				$iTemp = $oTask.State
				If $bReadable Then
					If $iTemp = $TASK_STATE_UNKNOWN Then __TS_TaskListWrite($aTasks, $iRow, $iColumn, $bReadable, "Unknown")
					If $iTemp = $TASK_STATE_DISABLED Then __TS_TaskListWrite($aTasks, $iRow, $iColumn, $bReadable, "Disabled")
					If $iTemp = $TASK_STATE_QUEUED Then __TS_TaskListWrite($aTasks, $iRow, $iColumn, $bReadable, "Queued")
					If $iTemp = $TASK_STATE_READY Then __TS_TaskListWrite($aTasks, $iRow, $iColumn, $bReadable, "Ready")
					If $iTemp = $TASK_STATE_RUNNING Then __TS_TaskListWrite($aTasks, $iRow, $iColumn, $bReadable, "Running")
				Else
...
;                  |2005 - Error setting a property. @extended is set to the COM error and the Returnvalue holds the negative index of the property in error ($aProperties)
; Author ........: water
; Modified.......:
...
; Syntax.........: _TS_TaskValidate($oService, $vTask)
; Parameters ....: $oService - Task Scheduler Service object as returned by _TS_Open
...
; Author ........: water
; Modified.......:
; Remarks .......: Set one of these three parameters to delete specific or all Triggers: $iIndex, $sID, $bDeleteAll.
;                  The parameters will be processed in the following sequence:
...
Func _TS_VersionInfo()
	Local $aVersionInfo[9] = [8, "V", 1, 6, 1.0, "20250126", "3.3.14.5", "water", "allow2010, AdmUL, water"]
	Return $aVersionInfo
...
;                  $sArguments        - [optional] Arguments passed to the executable file.
; Return values .: Success - 1
;                  Failure - Returns 0 and sets @error:
...
;                  $sDate          - [optional] Date when the Task was created. If not specified _NowCalc() is used (default)
...
;                  $sEnd            - [optional] The date and time when the Trigger will be deactivated. Format: YYYY-MM-DDTHH:MM:SS(+-)HH:MM.
;                                     default = "" which means: Trigger will never be deactivated. Please see Remarks.
...
; Syntax.........: _TS_Wrapper_TriggerLogon($oTaskDefinition, $iDelay, $sStart[, $sEnd = ""])
; Parameters ....: $oTaskDefinition - Task Definition object as returned by _TS_Wrapper_TaskCreate
;                  $iDelay          - Value in minutes indicating the time between the users logon and the start of the Task
;                  $sStart          - The date and time when the Trigger is activated. Format: YYYY-MM-DDTHH:MM:SS(+-)HH:MM. Please see Remarks.
;                  $sEnd            - [optional] The date and time when the Trigger will be deactivated. Format: YYYY-MM-DDTHH:MM:SS(+-)HH:MM.
;                                     default = "" which means: Trigger will never be deactivated. Please see Remarks.
; Return values .: Success - Object of the created trigger.
;                  Failure - Returns 0 and sets @error
...
; #INTERNAL_USE_ONLY#============================================================================================================
; Name ..........: __TS_ProcessParentPID
; Description ...: Determines the process ID (PID) of the parent process.
; Syntax.........: __TS_ProcessParentPID($iPID)
; Parameters ....: $iPID - Process ID (PID as integer) of the process whose calling process is to be determined.
; Return values .: Success - PID of the parent processt
;                  Failure - 0 and sets @error:
;                  |1 - Error connecting to WMI.
;                  |2 - Error getting process collection from WMI.
;                  |3 - No matching process found.
; Author ........: PsaltyDS
; Modified ......: water
; Remarks .......:
; Related .......: This function is a stripped down version of the code provided by PsaltyDS - Thanks!
; Link ..........: https://www.autoitscript.com/forum/topic/70538-_processlistproperties/
; Example .......: No
; ===============================================================================================================================
Func __TS_ProcessParentPID($iPID)
    Local $oWMI, $colProcs
    ; Convert PID if passed as string
    If StringIsInt($iPID) Then $iPID = Int($iPID)
    ; Connect to WMI and get $iPID objects
    $oWMI = ObjGet("winmgmts:{impersonationLevel=impersonate,authenticationLevel=pktPrivacy, (Debug)}!\\.\root\cimv2")
    If IsObj($oWMI) Then
        ; Get by PID
        $colProcs = $oWMI.ExecQuery("select * from win32_process where ProcessId = " & $iPID)
        If IsObj($colProcs) Then
            ; Return for no matches
            If $colProcs.count = 0 Then Return SetError(3, 0, 0)
            Return $colProcs.ItemIndex(0).ParentProcessId
        Else
            Return SetError(2, 0, 0)  ; Error getting process collection from WMI
        EndIf
    Else
        Return SetError(1, 0, 0) ; Error connecting to WMI
    EndIf
EndFunc   ;==>__ProcessParentPID