# *** WinRT by MattyD v1.5 (Preview) ***
# This project is still in early development. Expect bugs in some libraries!
# --------------------------------------------------------------------------
# Generated : 29 April 2025
# Platform  : WIN_11 23H2 22631
# File Type : Interface
# File Name : Windows.UI.Xaml.IUIElementStatics6
# Incl. In  : Windows.UI.Xaml.UIElement

#include-once
#include "..\WinRTCore.au3"
#include "IInspectable.au3"

Global Const $sIID_IUIElementStatics6 = "{647E03B7-036A-4DEA-9540-1DD7FD1266F1}"
$__g_mIIDs[$sIID_IUIElementStatics6] = "IUIElementStatics6"

Global Const $tagIUIElementStatics6 = $tagIInspectable & _
		"get_GettingFocusEvent hresult(ptr*);" & _ ; Out $pValue
		"get_LosingFocusEvent hresult(ptr*);" & _ ; Out $pValue
		"get_NoFocusCandidateFoundEvent hresult(ptr*);" ; Out $pValue

Func IUIElementStatics6_GetGettingFocusEvent($pThis)
	Local $vValue = __WinRT_GetProperty_Ptr($pThis, 7)
	Return SetError(@error, @extended, $vValue)
EndFunc

Func IUIElementStatics6_GetLosingFocusEvent($pThis)
	Local $vValue = __WinRT_GetProperty_Ptr($pThis, 8)
	Return SetError(@error, @extended, $vValue)
EndFunc

Func IUIElementStatics6_GetNoFocusCandidateFoundEvent($pThis)
	Local $vValue = __WinRT_GetProperty_Ptr($pThis, 9)
	Return SetError(@error, @extended, $vValue)
EndFunc
